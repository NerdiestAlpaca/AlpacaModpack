	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'satellite' | 'our-world' | 'mischief':
					curStage = 'pawtucket';
				case 'free-spirit' | 'catnap' | 'daredevil':
					curStage = 'park';
				case 'dreamboat' | 'primadonna' | 'swerve-on':
					curStage = 'littlestpetshop';
				case 'satellite-ex' | 'our-world-ex' | 'mischief-ex' | 'free-spirit-ex' | 'catnap-ex' | 'daredevil-ex' | 'dreamboat-ex' | 'primadonna-ex' | 'swerve-on-ex':
    				curStage = 'bfconcert';
				case 'wish' | 'purification' | 'chaos':
    				curStage = 'valley';
				case 'whimsy':
					curStage = 'highlands';
				case 'cyclone':
		...
		switch (curStage)
		{
			...
			case 'spooky': //Week 2
				...
				halloweenBG.setGraphicSize(Std.int(halloweenBG.width * 1.4));
				add(halloweenBG);
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				case 'pawtucket': //FNF AWOO - Week 1
				var bg:BGSprite = new BGSprite('petshop/bgWalls', -1000, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);
				
				if(!ClientPrefs.lowQuality) {
					upperBoppers = new BGSprite('petshop/upperBop', -240, -20, 0.33, 0.33, ['Upper Crowd Bob']);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					upperBoppers.updateHitbox();
					add(upperBoppers);
				
					var bgEscalator:BGSprite = new BGSprite('petshop/bgEscalator', -400, 0, 0.3, 0.3);
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					add(bgEscalator);
				
					var confetti:BGSprite = new BGSprite('petshop/confetti', -240, -90, 0.33, 0.33, ['confetti whoosh'], true);
					add(confetti);
				}

				bottomBoppers = new BGSprite('petshop/audienceBop', -700, 780, 1, 1, ['audience bob']);
				bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
				bottomBoppers.updateHitbox();
				
				var fgStage:BGSprite = new BGSprite('petshop/fgstage', -600, 700);
				add(fgStage);
			case 'park': 
					var bg:BGSprite = new BGSprite('petshop/outsidebg', -200, -100);
			...
			case 'littlestpetshop': 
					var bg:BGSprite = new BGSprite('petshop/NighttimeBG', -200, -100);
			...
			case 'bfconcert': //FNF AWOO - Week 1
				var bg:BGSprite = new BGSprite('petshop/stageback', -1000, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);
				...
				bottomBoppers = new BGSprite('petshop/BFAudience', -700, 780, 1, 1, ['audience bob']);
				bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
				bottomBoppers.updateHitbox();
				
				var fgStage:BGSprite = new BGSprite('petshop/stagefront', -600, 700);
				add(fgStage);

                var curtains:BGSprite = new BGSprite('petshop/stagecurtains', -1000, -500, 0.2, 0.2);
				curtains.setGraphicSize(Std.int(curtains.width * 0.8));
				curtains.updateHitbox();
				add(curtains);

				case 'highlands':
					var bg:BGSprite = new BGSprite('alpaca/bg', -550, -100, 0.3, 0.25, ['BG Phase 1'], true);
					bg.setGraphicSize(Std.int(bg.width * 1.7));
					bg.updateHitbox();
					add(bg);
				
					var fg:BGSprite = new BGSprite('alpaca/fg', -670, 550, 0.3, 0.6, ['Ground'], true);
					fg.setGraphicSize(Std.int(fg.width * 1.7));
					fg.updateHitbox();
					add(fg);
				
				case 'highlandsDemon':
					var bg:BGSprite = new BGSprite('alpacaRound2/bg', -550, -100, 0.3, 0.25, ['BG Phase 2'], true);
					bg.setGraphicSize(Std.int(bg.width * 1.7));
					bg.updateHitbox();
					add(bg);
				
					var fg:BGSprite = new BGSprite('alpacaRound2/fg', -670, 550, 0.3, 0.6, ['Ground'], true);
					fg.setGraphicSize(Std.int(fg.width * 1.7));
					fg.updateHitbox();
					add(fg);
				
				case 'hell':
					var bg:BGSprite = new BGSprite('thirdphase/bg', -1000, -500, 0.2, 0);
					bg.setGraphicSize(Std.int(bg.width * 3));
					bg.updateHitbox();
					add(bg);
				
					if(!ClientPrefs.lowQuality) {
						var fire:BGSprite = new BGSprite('thirdphase/fire', -300, -700, 0.1, 0.1, ['firewave'], true);
						fire.setGraphicSize(Std.int(fire.width * 1.2));
						fire.updateHitbox();
						add(fire);
					}
				...
				case 'realityhole':
					...
					if(!ClientPrefs.lowQuality) {
						var lightning:BGSprite = new BGSprite('finale/lightning', -800, -700, 0.45, 0.15, ['LightningThing'], true);
			...
			case 'valley':
				var bg:BGSprite = new BGSprite('valleyback', -600, -200, 0.8, 0.8);
				add(bg);

				var valleyFront:BGSprite = new BGSprite('valleyfront', -650, 600, 0.8, 0.8);
				valleyFront.setGraphicSize(Std.int(valleyFront.width * 1.1));
				valleyFront.updateHitbox();
				add(valleyFront);
		...
		case 'shroom':
			var bg:BGSprite = new BGSprite('shroomback', -600, -200, 0.9, 0.9);
			add(bg);

			var shroomFront:BGSprite = new BGSprite('shroomfront', -650, 600, 0.9, 0.9);
			shroomFront.setGraphicSize(Std.int(shroomFront.width * 1.1));
			shroomFront.updateHitbox();
			add(shroomFront);

			var shroomCurtains:BGSprite = new BGSprite('shroomcurtains', -500, -300, 1.3, 1.3);
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								health -= 0.16; //0.16 + 0.04 = -0.2 (-10%) of HP if you hit a hurt note
